...
public class StringHelper {
			...
			if ((letters && Character.isLetter(ch)) || (numbers && Character.isDigit(ch))
				...
				if (ch >= 56320 && ch <= 57343) {
					if (count == 0) {
						...
						// low surrogate, insert high
						// surrogate after putting it
						// in
						buffer[count] = ch;
						count--;
					...
					if (count == 0) {
						...
						// high surrogate, insert low
						// surrogate before putting
						// it in
					...
					// private high surrogate, no effing
					// clue, so skip it
					count++;
				} else {
					buffer[count] = ch;
	...
	/**
	 * Test whether the given string matches the given substring at the
	 * given index.
	 * 
	 * @param str
	 *                the original string (or StringBuilder)
	 * @param index
	 *                the index in the original string to start matching
	 *                against
	 * @param substring
