	...
	private void initOtherProperties() {
		...
		//TODO sl_列注释判断设置是否生成日期格式化
		  ...
		  if ("DATETIME".equals(_sqlTypeName)&&_sqlType==93&&_remarks!=null) {
				int idx=_remarks.indexOf("type=");
				if (idx==-1) {
					return ;
				}
				String type=_remarks.substring(idx+5, idx+6);
				if ("1".equals(type)) {//
					System.out.println("========需要日期格式化：表:"+_table.getSqlName()+",列："+columnName+",remarks:"+_remarks);
					int idxFmtStart=_remarks.indexOf("&")+5;
					int idxFmtEnd=_remarks.indexOf("]", idx);
					this.format=_remarks.substring(idxFmtStart,idxFmtEnd);
					_table.setNeedDateTimeFormat(true);
					setNeedDateTimeFormat(true);
					needDateTimeFormat=true;
				}
			}
	}
